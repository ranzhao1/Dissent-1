...
namespace BlogDrop {
  ...
  HashingGenClientCiphertext::PairingClientCiphertext(const QSharedPointer<const Parameters> params, 
  ...
  void HashingGenClientCiphertext::InitCiphertext(int phase, 
      const QSharedPointer<const PrivateKey> client_priv) 
  {
    for(int i=0; i<GetNElements(); i++) { 
      Element base = BlogDropUtils::GetHashedGenerator(_params, 
          GetAuthorKey(), phase, i); 

      // _element[i] = gi^client_priv
      _elements.append(_params->GetMessageGroup()->Exponentiate(base, client_priv->GetInteger())); 
    }
  }

  void HashingGenClientCiphertext::SetAuthorProof(int phase, 
      const QSharedPointer<const PrivateKey> client_priv, 
  ...
  void HashingGenClientCiphertext::SetProof(int phase, QSharedPointer<const PrivateKey> client_priv)
  {
    InitCiphertext(phase, client_priv);
  ...
  bool HashingGenClientCiphertext::VerifyProof(int phase,
  ...
  QByteArray HashingGenClientCiphertext::GetByteArray() const 
  {
    QList<QByteArray> list;
  ...
  void HashingGenClientCiphertext::InitializeLists(
  ...
  Integer HashingGenClientCiphertext::Commit(const QSharedPointer<const Parameters> &params,
