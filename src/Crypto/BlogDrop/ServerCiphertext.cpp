...
namespace BlogDrop {
  ...
  {
    Q_ASSERT(pubs.count() == server_ctexts.count());
    ...
    if(tt == CryptoFactory::SingleThreaded) {
      ...
      for(int server_idx=0; server_idx<server_ctexts.count(); server_idx++) {
        list.append(CiphertextFactory::CreateServerCiphertext(params, 
              server_pk_set, author_pk, client_ctexts, server_ctexts[server_idx]));
      }

      // Verify each proof
      for(int idx=0; idx<server_ctexts.count(); idx++) {
      ...
      for(int server_idx=0; server_idx<server_ctexts.count(); server_idx++) {
        ...
        m->server_ciphertext = server_ctexts[server_idx];
        m->phase = phase;
      ...
      for(int server_idx=0; server_idx<valid_list.count(); server_idx++) {
        if(valid_list[server_idx]) {
          c_out.append(CiphertextFactory::CreateServerCiphertext(
              params, server_pk_set, author_pk, client_ctexts, server_ctexts[server_idx]));
        }
      }
