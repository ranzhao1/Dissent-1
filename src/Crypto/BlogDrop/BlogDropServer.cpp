...
namespace BlogDrop {
  ...
  {
    if(!in.count()) {
      qWarning() << "Added empty client ciphertext list";
      return true;
    }
  ...
  {
    if(!_client_pubs.count()) {
      qWarning() << "Closed bin that contains no ciphertexts";
      return QByteArray();
    }
  ...
  {
    if(!in.count()) {
      qWarning() << "Added empty server ciphertext list";
      return true;
    }
