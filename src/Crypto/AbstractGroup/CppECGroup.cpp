...
namespace AbstractGroup {
  ...
  {
   /*
    * We add a leading and trailing 0xff byte to the QByteArray,
    * plus one extra "counter" byte, then we interpret that string
    * as an x-coordinate. If there is a y value such that (x,y)
    * is on the curve, we return that point. Otherwise, we increment the 
    * "counter" byte and try again.
    *
    * If we're using 8-bit bytes, then our probability of failure
    * is close to 1/(2^(2^8)) = (1/2)^256
    */
