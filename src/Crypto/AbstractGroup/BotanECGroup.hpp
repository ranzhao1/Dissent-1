#ifndef DISSENT_CRYPTO_ABSTRACT_GROUP_BOTAN_EC_GROUP_H_GUARD
#define DISSENT_CRYPTO_ABSTRACT_GROUP_BOTAN_EC_GROUP_H_GUARD
...
namespace AbstractGroup {
   ...
   *   y^2 = x^3 + ax + b (mod p)
   */
  class BotanECGroup : public AbstractGroup {
    ...
    public:
       ...
       */
      BotanECGroup(Integer p, Integer q,
      ...
      inline virtual Element GetGenerator() const { 
        return Element(new BotanECElementData(_g)); 
      }
      ...
      inline virtual Element GetIdentity() const { 
        return Element(new BotanECElementData(Botan::PointGFp(_curve))); 
      }
       ...
       */
      inline Integer GetFieldSize() const { return FromBotanInt(_curve.get_p()); }
    ...
    private:
      ...
      Botan::PointGFp GetPoint(const Element &e) const;

      inline static Botan::BigInt ToBotanInt(const Integer &e) 
      {
        // Hex encoding does not include minus sign        
        QByteArray bytes = e.GetByteArray();
        Botan::BigInt i = Botan::BigInt::decode(
            reinterpret_cast<const byte*>(bytes.constData()), bytes.count());
        if(e < 0) i.set_sign(Botan::BigInt::Negative);
        return i;
      }

      inline static Integer FromBotanInt(const Botan::BigInt &i)
      {
        QByteArray bytes(i.encoded_size(), 0);
        i.binary_encode(reinterpret_cast<byte*>(bytes.data()));
        return Integer(new CppIntegerData(bytes));
      }
       ...
       */
      bool SolveForY(const Botan::BigInt &x, Element &point) const;

      Botan::CurveGFp _curve;
      Integer _q;
      Botan::PointGFp _g;
